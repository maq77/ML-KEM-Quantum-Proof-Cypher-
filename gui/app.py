...
class MLKEMGuiApp:
    def __init__(self, root: tk.Tk):
        ...
        # File paths (bound to Entry widgets)
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            main,
            text="ML-KEM-512 + SimpleStreamCipher (from-scratch core, C++ + Python GUI)",
        ...
        ttk.Button(main, text="1) Generate new ML-KEM-512 keypair", command=self.on_keygen).grid(
            row=2, column=0, sticky="w", pady=(2, 2)
        )

        ttk.Button(main, text="2) Save keypair...", command=self.on_save_keypair).grid(
            row=2, column=1, sticky="w", padx=(5, 0)
        )

        ttk.Button(main, text="3) Load keypair...", command=self.on_load_keypair).grid(
            row=2, column=2, sticky="w", padx=(5, 0)
        )

        ttk.Entry(main, textvariable=self.keyfile_var, width=40).grid(
            row=2, column=3, sticky="ew", padx=(5, 0)
        )

        self.key_status = ttk.Label(main, text="Keys: not loaded", foreground="red")
        self.key_status.grid(row=3, column=0, columnspan=4, sticky="w", pady=(2, 5))
        ...
        # --- Encrypt section ---
        ttk.Label(main, text="Encrypt (Encapsulate + Stream Cipher):", font=("Segoe UI", 10, "bold")).grid(
            row=5, column=0, columnspan=4, sticky="w"
        )

        ttk.Button(main, text="4) Choose plaintext file...", command=self.on_choose_plain).grid(
            row=6, column=0, sticky="w", pady=(3, 2)
        )
        ttk.Entry(main, textvariable=self.plain_in_var, width=60).grid(
            row=6, column=1, columnspan=3, sticky="ew", padx=(5, 0)
        )

        ttk.Button(main, text="5) Encrypt → Save bundle.json", command=self.on_encrypt).grid(
            row=7, column=0, sticky="w", pady=(3, 2)
        )
        ...
        # --- Decrypt section ---
        ttk.Label(main, text="Decrypt (Decapsulate + Stream Cipher):", font=("Segoe UI", 10, "bold")).grid(
            row=9, column=0, columnspan=4, sticky="w"
        )

        ttk.Button(main, text="6) Choose bundle.json...", command=self.on_choose_bundle).grid(
            row=10, column=0, sticky="w", pady=(3, 2)
        )
        ttk.Entry(main, textvariable=self.bundle_in_var, width=60).grid(
            row=10, column=1, columnspan=3, sticky="ew", padx=(5, 0)
        )

        ttk.Button(main, text="7) Decrypt → Save plaintext", command=self.on_decrypt).grid(
            row=11, column=0, sticky="w", pady=(3, 2)
        )
        ...
        status_label.grid(row=12, column=0, columnspan=4, sticky="w", pady=(8, 0))
    ...
    def on_keygen(self):
        ...
        self.key_status.config(text="Keys: generated (not yet saved)", foreground="green")
        self.status_var.set("Generated new ML-KEM-512 keypair.")
    ...
    def on_save_keypair(self):
        ...
        try:
            ...
            self.key_status.config(text=f"Keys: saved ({os.path.basename(path)})", foreground="green")
    ...
    def on_load_keypair(self):
        ...
        try:
            ...
            self.keyfile_var.set(path)
            self.key_status.config(text=f"Keys: loaded ({os.path.basename(path)})", foreground="green")
    ...
    def on_encrypt(self):
        ...
        try:
            ...
            self.status_var.set(f"Encrypted and saved bundle to {out_path}")
            messagebox.showinfo("Success", "Encapsulation + encryption complete.")
        except Exception as e:
    ...
    def on_decrypt(self):
        ...
        try:
            ...
            self.status_var.set(f"Decrypted plaintext saved to {out_path}")
            messagebox.showinfo("Success", "Decapsulation + decryption complete.")
        except Exception as e:
